    ...
    <div class="col-8 px-0">
        <div class="px-3 pb-4 pt-0 mx-4 mb-5">
            <div class="p-3 rounded-top standard-color-blue">
            	<h4 class="text-white my-0 d-inline"> 
            		Legislação da ONG  
            	</h4>
            	
            	<c:if test="${legislationFiles.size() > 0}">
            		<form method="GET" action="<c:url value='/legislation/download'/>" class="float-right"> 
            ...
            <div class="card border-top-0">
                <div class="card-block">
                    <div class="m-0 pt-1 pb-0">
                    	<c:choose>
                    		<c:when test="${legislationFiles.size() > 0}">
                  				<c:forEach items="${legislationFiles}" var="eachFile">
                  					<div class="w-100 embed-responsive embed-responsive-1by1 my-3">
	                    				<iframe class="embed-responsive-item" src="<c:url value='/legislation/${eachFile.getId()}/view'/>"> </iframe>
	                    			</div>
                  				</c:forEach>
                    		</c:when>
                    		<c:otherwise> 
